...
bot = telebot.TeleBot('5914448938:AAHHSuGitqd3h1a0IX8pSpK-Pk-9ZH_AiZk')
bot.remove_webhook()

@bot.message_handler(commands=['start'])
def send_wel(message):
    inline = types.InlineKeyboardMarkup(row_width=3)
    make = types.InlineKeyboardButton("Create from fake mail",callback_data="start")
    pr = types.InlineKeyboardButton("Create from a private email",callback_data="ptmail")
    inline.add(make,pr)
    bot.send_message(chat_id=(message.chat.id),text=f'*Welcome { message.from_user.first_name} to the bot making free hosting \nBy @X_D_X_D*',reply_markup=inline,parse_mode='markdown')
...
''',parse_mode='html')
        ...
        bot.edit_message_text(chat_id=(call.message.chat.id),message_id=(call.message.id),text="send the email")
